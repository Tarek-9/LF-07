...
async function getById(id, conn = pool) {
  // TABLE: sensor
  const [rows] = await conn.query(`SELECT * FROM sensor WHERE id = :id`, {
    id,
  });
  return rows[0] || null;
}
...
async function listAll({ typ, spindId, aktiv } = {}, conn = pool) {
  ...
  if (spindId != null) {
    where.push(`spind_id = :spindId`);
    params.spindId = Number(spindId);
  }
  if (aktiv != null) {
    where.push(`aktiv = :aktiv`);
...
async function update({ id, typ, aktiv, spind_id }, conn = pool) {
  ...
  if (aktiv !== undefined) {
    sets.push(`aktiv = :aktiv`);
    params.aktiv = aktiv ? 1 : 0;
  }
  if (spind_id !== undefined) {
    sets.push(`spind_id = :spind_id`);
    params.spind_id = spind_id;
  }

  if (!sets.length) return getById(id, conn);
  // TABLE: sensor
  await conn.query(
    `UPDATE sensor SET ${sets.join(', ')} WHERE id = :id`,
    params
  );
...
async function setAktiv({ id, aktiv }, conn = pool) {
  // TABLE: sensor
  await conn.query(`UPDATE sensor SET aktiv = :aktiv WHERE id = :id`, {
    id,
    aktiv: aktiv ? 1 : 0,
  });
  return getById(id, conn);
...
async function toggleAktiv(id, conn = pool) {
  // TABLE: sensor
  await conn.query(`UPDATE sensor SET aktiv = NOT aktiv WHERE id = :id`, {
    id,
  });
  return getById(id, conn);
}
