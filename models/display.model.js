...
async function getById(id, conn = pool) {
  // TABLE: display
  const [rows] = await conn.query(`SELECT * FROM display WHERE id = :id`, {
    id,
  });
  return rows[0] || null;
}
...
async function update({ id, typ, inhalt, spind_id }, conn = pool) {
  ...
  // TABLE: display
  await conn.query(
    `UPDATE display SET ${sets.join(', ')} WHERE id = :id`,
    params
  );
...
async function updateContent({ id, inhalt }, conn = pool) {
  // TABLE: display
  await conn.query(`UPDATE display SET inhalt = :inhalt WHERE id = :id`, {
    id,
    inhalt,
  });
  return getById(id, conn);
