...
async function listLockers(req, res) {
  try {
    ...
    const rows = await Lockers.getAll({
      status:
        status && ['frei', 'reserviert', 'belegt'].includes(status)
          ? status
          : undefined,
...
async function reserveLocker(req, res) {
  try {
    ...
    const result = await Lockers.reserveLocker({
      lockerId: id,
      userId,
      minutes,
    });
    if (!result.ok) {
      ...
      if (result.code === 'NOT_AVAILABLE')
        return res.status(409).json({ message: 'Spind nicht verfügbar.' });
...
async function occupyLocker(req, res) {
  try {
    ...
    if (!result.ok) {
      if (result.code === 'NOT_FOUND')
        return res.status(404).json({ message: 'Spind nicht gefunden.' });
      if (result.code === 'RESERVED_BY_OTHER')
        return res
          .status(409)
          .json({ message: 'Spind ist für jemand anderen reserviert.' });
      if (result.code === 'ALREADY_OCCUPIED')
        return res.status(409).json({ message: 'Spind bereits belegt.' });
  ...
  try {
    ...
    if (!result.ok) {
      ...
      if (result.code === 'NOT_OWNER')
        return res
          .status(403)
          .json({ message: 'Nur der Besitzer oder Admin darf freigeben.' });
