...
return res.status(400).json({ message: 'username, email, password erforderlich' });
}
...
if (byName) return res.status(409).json({ message: 'username existiert bereits' });
if (byMail) return res.status(409).json({ message: 'email existiert bereits' });
...
if (!username || !email) {
return res.status(400).json({ message: 'username und email erforderlich' });
}
