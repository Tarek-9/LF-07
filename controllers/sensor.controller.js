...
function validatePayload({ typ, aktiv, spind_id }, { partial = false } = {}) {
    const errors = [];
    if (!partial) {
        if (!typ || !Sensor.isValidType(typ)) errors.push('typ ist erforderlich und muss PIR, RFID oder PIN sein.');
...
async function getSensor(req, res) {
    try {
        ...
        if (!row) return res.status(404).json({ message: 'Sensor nicht gefunden.' });
...
async function createSensor(req, res) {
    try {
        ...
        if (err.code === 'INVALID_TYPE') {
            return res.status(400).json({ message: 'typ muss PIR, RFID oder PIN sein.' });
        }
...
async function updateSensor(req, res) {
    try {
        ...
        if (!exists) return res.status(404).json({ message: 'Sensor nicht gefunden.' });
...
async function setSensorAktiv(req, res) {
    try {
        ...
        if (typeof aktiv !== 'boolean') return res.status(400).json({ message: 'aktiv muss boolean sein.' });
...
async function upsertSpindSensor(req, res) {
    try {
        ...
        const errors = validatePayload({ typ, aktiv, spind_id }, { partial: false });
        if (errors.length) return res.status(400).json({ message: errors.join(' ') });

        const row = await Sensor.upsertBySpindAndType({ spind_id, typ, aktiv: aktiv ?? true });
        return res.status(200).json(row);
    } catch (err) {
        if (err.code === 'INVALID_TYPE') {
            return res.status(400).json({ message: 'typ muss PIR, RFID oder PIN sein.' });
