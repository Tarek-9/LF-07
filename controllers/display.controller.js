...
function validatePayload({ typ, inhalt, spind_id }, { partial = false } = {}) {
  ...
  if (!partial) {
    if (!typ || !Display.isValidType(typ))
      errors.push('typ ist erforderlich und muss 7segment oder lcd1602 sein.');
...
async function createDisplay(req, res) {
  try {
    ...
    if (err.code === 'INVALID_TYPE') {
      return res
        .status(400)
        .json({ message: 'typ muss 7segment oder lcd1602 sein.' });
    }
...
async function updateDisplayContent(req, res) {
  try {
    ...
    if (inhalt.length > 255)
      return res
        .status(400)
        .json({ message: 'inhalt zu lang (max. 255 Zeichen).' });
...
async function upsertSpindDisplay(req, res) {
  try {
    ...
    const row = await Display.upsertBySpindAndType({
      spind_id,
      typ,
      inhalt: inhalt ?? null,
    });
